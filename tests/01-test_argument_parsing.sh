...
check_message_in_output 'ansible-deployer -t task_exec_bin_true -i testInfra' '\[CRITICAL\]: First positional argument (subcommand) is required! Available commands are: run, list, lock, unlock.'
...
check_run_ok 'ansible-deployer verify --dry -t task_exec_bin_true -s prod -i testInfra'
check_run_ok "ansible-deployer run --dry -t task_with_limit -s testing -i testInfra -l testHost1"
check_run_ok "ansible-deployer verify --dry -t task_with_limit -s testing -i testInfra -l testHost1"
