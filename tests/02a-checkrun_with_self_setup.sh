...
check_message_not_in_output "ansible-deployer verify --task=task_exec_bin_true -s testing -i testInfra -d --self-setup /etc/alt-workdir" "\[INFO\]: Setup completed in /etc/alt-workdir"
search_path=$(find_latest_sequence)
check_file_startingwith_in_dir "/etc/alt-workdir" "runBin.yaml"
check_file_startingwith_in_dir "$search_path" "ansible-deploy_execution_"
