...
def create_workdirs(timestamp: str, conf: dict, logger):
    ...
    # Set hard counter to 5 or in globalvars, or in etc/ansible-deploy.yaml
    return create_workdir(base_dir, conf, logger, 5)

def create_workdir(base_dir: str, conf: dict, logger, repeat_counter: int):
    """
    Function to create working directory on file system, we expect it to change
    the cwd to newly created workdir at the end.
    """
    for cnt in range(0, repeat_counter):
    ...
    logger.critical("Failed to create work dir: %s . All %d retry attempts were utilized.",
                    seq_path, repeat_counter)
    sys.exit(91)
